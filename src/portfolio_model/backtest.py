...
def value_proxy_score(features: StrategyFeatures) -> float:
    return 0.55 * clamp(-features.drawdown, 0.0, 0.35) / 0.35 + 0.45 * (1.0 - clamp(features.volatility, 0.0, 0.8) / 0.8)
...
def quality_proxy_score(features: StrategyFeatures) -> float:
    return 0.60 * (clamp(features.reward_to_risk, -1.0, 3.0) + 1.0) / 4.0 + 0.40 * (1.0 - clamp(features.volatility, 0.0, 0.8) / 0.8)
...
def validate_forward_estimates(forward_estimates: dict[str, list[ForwardEstimate]]) -> None:
    if not forward_estimates:
        raise RuntimeError(
            "forward_pe_value requires --forward-estimates with point-in-time forward P/E, "
            "forward EPS growth, and estimate revision data."
        )
...
def latest_forward_estimate(
    ...
    latest = None
    for estimate in forward_estimates.get(symbol.upper(), []):
        if estimate.day > day:
            break
        latest = estimate
    return latest
